...
int fs_create(const char *filename)
{
	...
	for (i = 0; i < 128; ++i) {
		if (rootFolder[i].filename == NULL) {
			rootFolder[i].filename == &filename;
			rootFolder[i].fileSize == 0;
			rootFolder[i].dataIndex == "FAT_EOC";
			return 0;
		}
	}
}
...
int fs_delete(const char *filename)
{
	...
	for (i = 0; i < 128; ++i) {
		...
		for (i = 0; i < 128; ++i) {
			if (rootFolder[i].filename == &filename) {
				rootFolder[i].filename == NULL;
				rootFolder[i].fileSize == NULL;
				rootFolder[i].dataIndex == NULL;
			}
		}
		return 0;
	}
